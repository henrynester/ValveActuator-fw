...
void AutoPID::run()
{
  ...
  {                                           //otherwise use PID control
    ...
    //if (_dT >= _timeStep)
    //{ //if long enough, do PID calculations (I removed this timing from this code since I want to run it in my own timed loop)
    _lastStep = millis();
    double _error = *_setpoint - *_input;
    _integral += (_error + _previousError) / 2 * _dT / 1000.0;            //Riemann sum integral
    _integral = constrain(_integral, _outputMin / _Ki, _outputMax / _Ki); //uncomment to reduce integral windup
